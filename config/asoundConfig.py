...
def setSpeakerInfo(hw):
  if debug: print("set speak info:"+str(hw['Speaker'])+" " + hw['SpeakerBrand'])
...
def makeRc():
  path = "%s/%s/asoundrc.template"%(home,config.specs['asoundDir'])
  ...
  try:
    ...
    with open(path) as f:
      for line in f:
        ...
        if debug: print("writing line:"+line);
        rc.write(line)
  except Exception, e:
